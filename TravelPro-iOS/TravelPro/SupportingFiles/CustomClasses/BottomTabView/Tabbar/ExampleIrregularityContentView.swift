...
class ExampleIrregularityContentView: ESTabBarItemContentView {
    ...
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let pVal = CGPoint.init(x: point.x - imageView.frame.origin.x, y: point.y - imageView.frame.origin.y)
        return sqrt(pow(imageView.bounds.size.width / 2.0 - pVal.x, 2) + pow(imageView.bounds.size.height / 2.0 - pVal.y, 2)) < imageView.bounds.size.width / 2.0
    }
